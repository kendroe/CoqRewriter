...
let get_env_terms env vars =
    ...
    in
        (List.filter binary_term
            (List.map
                (fun (x) -> (match x with
                                    | (APPL (f,[l;(APPL (4,[]));c])) -> Context.unmarkVars l
                                    | (APPL (f,[l;(APPL (5,[]));c])) -> (APPL (intern_not,[Context.unmarkVars l]))
                                    | (APPL (f,[l;(APPL (17,[APPL (5,[])]));c])) -> Context.unmarkVars l
                                    | (APPL (f,[l;(APPL (17,[APPL (4,[])]));c])) -> (APPL (17,[Context.unmarkVars l]))))
               (List.filter
                    (fun (x) -> not((Mylist.intersect (Context.getMarkedVars x) vars)=[])) rules)))
    ;;
...
let rec rewrite_op (t,c,e) env cond_terms terms =
    ...
    let results2 = elaborate_phase env (t,c,e) (terms@cond_terms) in
        ...
        val _ = flush_out std_out*)
    let _ = Trace.trace_list "kbrewrite" (fun (e) -> List.map (fun (x) -> ("results1 " ^ (prExp x))) results1) in
    let _ = Trace.trace_list "kbrewrite" (fun (e) -> List.map (fun (x) -> ("results2 " ^ (prExp x))) results2) in
...
let main_rewrite_loop rewrite env exp =
    ...
    let _ = Trace.trace_list "kbrewrite" (fun (e) -> List.map (fun (x) -> ("env_term " ^ (prExp x))) env_terms) in
    let env_op_terms =
    ...
    let res = List.map (fun (x) -> (match (x@residue) with
                      ...
                      (List.fold_right
                          ...
                          ops [exp_terms]) in
    let _ = Trace.trace "kbrewrite" (fun (xx) -> "Results: " ^ (prExp exp)) in
    let _ = Trace.indent () in
