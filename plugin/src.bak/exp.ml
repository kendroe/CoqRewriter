...
and prLet t = match t with
  ...
  | e -> " IN " ^ (prExp e)
and prBinding v t e =
    if Rtype.notype=t then (prExp v) ^ "=" ^ (prExp e)
                   else (prExp v) ^ ":" ^ (Rtype.unparse t) ^ "=" ^
                        (prExp e)
and prBound t = match t with
  | [] -> ""
  | ((a,t)::b::c) ->
    if Rtype.notype=t then (Intern.decode a) ^ ", " ^ (prBound (b::c))
                   else (Intern.decode a) ^ ":" ^ (Rtype.unparse t) ^ ", " ^
                        (prBound (b::c))
  | [(a,t)] ->
    if Rtype.notype=t then (Intern.decode a)
                   else (Intern.decode a) ^ ":" ^ (Rtype.unparse t)
...
let rec factor l =
            ...
            ||| (((((getToken (ID "CASE")) & exp) & (getToken (ID "OF"))) &
                cases) ==> (fun (((a,b),c),d) -> (CASE (b,Rtype.notype,d))))
            ...
            ||| (((((getToken (ID "fn")) & (getToken (ID ""))) &
                (getToken (SYMBOL "=>"))) & exp) ==>
                (fun (((a,ID b),c),d) ->
                    QUANT (Intern.intern_lambda,[(Intern.intern b,Rtype.notype)],d,(APPL (Intern.intern_true,[])))))
...
and varList l = (((((getToken (ID "")) & (getToken (SPECIAL ","))) & varList)
                    ==> (fun ((ID i,x),l) -> ((Intern.intern i,Rtype.notype)::l)))
             ||| ((getToken (ID "") ==> (fun (ID x) -> [(Intern.intern x,Rtype.notype)])))) l
...
and buildLet l e = match l with
  | [] -> e
  | ((v,e)::r) -> LET (v,Rtype.notype,e,buildLet r e)
