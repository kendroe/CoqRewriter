...
let rec buildIndices f exp e u = match e with
  | (VAR x) -> Rsubst.addPair u x (makeIndex f exp)
  | (APPL (s,l)) ->
    let (_,u) =
...
let reduceLet (LET (v,t,e,p)) = Rsubst.subst (buildIndices [] e v Rsubst.empty) p ;;
...
and uu_equal env e1 e2 q = match (e1,e2) with
  ...
  | ((CASE (e1,t1,c1)),(CASE (e2,t2,c2))) ->
    let l1 = List.map (fun (p,e) -> Rsubst.subst (buildIndices [] e p Rsubst.empty) e1) c1 in
    let l2 = List.map (fun (p,e) -> Rsubst.subst (buildIndices [] e p Rsubst.empty) e2) c2 in
    ...
    List.fold_left List.append []
        (List.map (fun (e,r) ->
           if List.length e = 0 then
               ...
               [(Rsubst.addPair theta v (APPL (s,e)),r)]) (all_splits rest)) ;;
    ...
    let d = depth1 v q in
        if d = -1 then
            (if no_bound e q then
                (if (List.mem v (Rsubst.dom theta)) then
                    List.map
                        (fun (x) -> (theta,x))
                    ...
                    match e with
                      | (APPL (s,l)) -> if (Renv.isAC env s) then
                                             ...
                                             [(Rsubst.addPair theta v e,[])]
                      | _             -> [(Rsubst.addPair theta v e,[])])
            else
                [])
        else
            (match e with
...
and match_multiple_var env v terms n s q theta =
    if n=1 then
        ...
        (if List.mem v (Rsubst.dom theta) then
             ...
             (List.map
                 (fun (p,r) -> (Rsubst.addPair theta v (APPL (s,p)),r))
                 (all_n_picks terms n)))
  ...
  | ((REF p),(REF e)) ->
   (try (Cache.get_match p e) with Cache.NoEntry ->
    ...
    in
        ...
        r)
  | (p,e) ->
    let _ = Rtrace.trace "rewriteRule" (fun (x) -> "match " ^ (prExp p) ^ " " ^ (prExp e)) in
    let _ = Rtrace.indent () in
    let res = u_match env p e [] Rsubst.empty in
    let _ = Rtrace.undent () in
    let _ = Rtrace.trace "rewriteRule" (fun (x) -> "end match " ^ (prExp p) ^ " " ^ (prExp e)) in
        res
    ;;
...
let rec u_unify env t1 t2 q theta1 theta2 = match (t1,t2) with
  ...
  | ((VAR v1),(VAR v2)) ->
    ...
    let d2 = depth3 v2 q in
        if d1 = -1 && d2= -1 then
           (if (List.mem v1 (Rsubst.dom theta1)) then
                List.map (fun (t2,r) -> (theta1,t2,r,[]))
                    (u_match env (VAR v2) (Rsubst.apply theta1 v1) (swap q) theta2)
            else if (List.mem v2 (Rsubst.dom theta2)) then
                List.map (fun (t1,r) -> (t1,theta2,[],r))
                    (u_match env (VAR v1) (Rsubst.apply theta2 v2) q theta1)
            else
                [(Rsubst.addPair theta1 v1 (VAR v2),Rsubst.addPair theta2 v2 (VAR v2),[],[])])
        else
  ...
  | ((VAR v),e) ->
    let d = depth1 v q in
        if d = -1 then
            (if no_bound e q then
                (if (List.mem v (Rsubst.dom theta1)) then
                    List.map
                    ...
                    match e with
                      | (APPL (s,l)) -> if (Renv.isAC env s) then
                                             ...
                                             [(Rsubst.addPair theta1 v e,theta2,[],[])]
                      | _             -> [(Rsubst.addPair theta1 v e,theta2,[],[])])
            else
                [])
        else []
  | (e,(VAR v)) ->
    let d = depth3 v q in
        if d = -1 then
            (if no_bound e q then
                (if (List.mem v (Rsubst.dom theta2)) then
                    List.map
                        (fun (x) -> (theta1,theta2,[],x))
                    ...
                    match e with
                      | (APPL (s,l)) -> if (Renv.isAC env s) then
                                             ...
                                             [(theta1,Rsubst.addPair theta2 v e,[],[])]
                      | _             -> [(theta1,Rsubst.addPair theta2 v e,[],[])])
            else
                [])
        else []
  | ((MARKED_VAR v1),(MARKED_VAR v2)) ->
...
and unify_multiple_var1 env v terms n s q theta1 theta2 =
    ...
    in
        if n=1 then
            ...
            (if List.mem vv (Rsubst.dom theta1) then
                 []
             else
                 (List.map
                     (fun (p,r) -> (Rsubst.addPair theta1 vv (APPL (s,p)),theta2,r,[]))
                     (all_n_picks terms n)))
...
and unify_multiple_var2 env terms v n s q theta1 theta2 =
    ...
    in
        if n=1 then
            ...
            (if List.mem vv (Rsubst.dom theta2) then
                 []
             else
                 (List.map
                     (fun (p,r) -> (theta1,Rsubst.addPair theta2 vv (APPL (s,p)),[],r))
                     (all_n_picks terms n)))
...
let rec unify env p e = match (p,e) with
  ...
  | (p,e) ->
    let _ = Rtrace.trace "rewriteRule" (fun (x) -> "unify " ^ (prExp p) ^ " " ^ (prExp e)) in
    let _ = Rtrace.indent () in
    let res = u_unify env p e [] Rsubst.empty Rsubst.empty in
    let _ = Rtrace.undent () in
    let _ = Rtrace.trace "rewriteRule" (fun (x) -> "end unify " ^ (prExp p) ^ " " ^ (prExp e))
    in
        res
    ;;
...
let rec remove_equal_dups env l = match l with
  | [] -> []
  | (a::b) ->
    ((Rtrace.trace "rewriteRule" (fun (xx) -> "equal_dups " ^ (string_of_int (List.length (a::b))))) ;
    a::(remove_equal_dups env (remove_ed env a b)))
  ;;

let rec swallow_symbols env s exps t =
    let (*val _ = Rtrace.trace "match" (fn (x) => ("Breaking " ^ (decode s) ^ " " ^ (prExp t)))
    let _ = Rtrace.trace_list "match" (fn (x) => (map prExp exps))*)
        matches = List.map
                          (fun (a,b) -> List.map (Rsubst.apply a) (Rsubst.dom a))
                          (List.filter
                              (fun (a,b) -> b=[])
                              (List.fold_left List.append [] (List.map
                                  (fun (x) -> thematch env x t) exps))) in
        (*val _ = Rtrace.trace "match" (fn (x) => "Matches:")
    let _ = Rtrace.trace_list "match" (fn (x) => (map prExp (foldr append [] matches)))*)
...
and es env p e = match (p,e) with
  | ((VAR x),_) -> List.mem x (Rcontext.getFreeVars e)
  | ((MARKED_VAR x),_) ->
    if (Rcontext.getFreeVars e)=[] then
        List.mem x (Rcontext.getMarkedVars e)
    else
        true
  | ((NUM n),(APPL (s,l))) -> true
  | ((NUM n),(NUM m)) -> not(m<n)
