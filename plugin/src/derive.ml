...
let rec add_types l = match l with
  | [] -> []
  | (a::b) -> (a,Rtype.notype)::(add_types b)
  ;;
...
let rec mk_and_conds c s l l2 = match l2 with
  ...
  | (a::b) ->
    ...
    let vars = Mylist.difference (Rcontext.getFreeVars cond) (Rcontext.getFreeVars a) in
...
let rec derive_rules e = match e with
  ...
  | (APPL (s,[(QUANT (14,t,e,p));NORMAL (APPL(4,[]));c])) ->
    let e1 = Rcontext.unmarkVarsList e (strip t) in
    let p1 = Rcontext.unmarkVarsList p (strip t)
    in
...
let rec derive env e = match e with
  ...
  | (APPL (f,[l;r;c])) ->
    ...
    let _ = Rtrace.indent () in
    let _ = Rtrace.trace_list "derive" (fun (x) -> List.map prExp res) in
    let _ = (Rtrace.undent () ; Rtrace.undent ()) in
    let _ = Rtrace.trace "derive" (fun (xx) -> "end derive for: " ^ (prExp e)) in
        res ;;

let rec rule_add_derive env (APPL (f,[l;r;c])) =
    let (APPL (f,[l;r;c])) = map_def (APPL (f,[l;r;c])) in
    let e = (APPL (f,[l;r;c])) in
    let _ = Rtrace.trace "derive" (fun (xx) -> "RA Deriving " ^ (prExp e)) in
    let _ = Rtrace.indent () in
    let user_rules = make_user_rules env in
    let user_disc = List.fold_right (fun (oo,n) -> (fun d ->
Disc.add (Renv.isAorC env) d (APPL (intern_oriented_rule,[oo;n;(APPL (intern_true,[]))]))
    ...
    let _ = (Rtrace.undent () ; Rtrace.undent ()) in
    let _ = Rtrace.trace "derive" (fun (xx) -> "end derive for: " ^ (prExp e))
    in
        res
    ;;
