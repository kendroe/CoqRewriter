...
let bool_type = Rtype.parse "Bool" ;;
...
let rec find_equal vars l l2 = match l2 with
  ...
  | ((APPL (11,[VAR v;x]))::r) ->
    if List.mem v vars && not(List.mem v (Rcontext.getFreeVars x)) then
...
let rec make_subst vars e = match e with
  | (APPL (11,[VAR v1;VAR v2])) ->
    if List.mem v1 vars then
        Rsubst.addPair Rsubst.empty v1 (VAR v2)
    else
        Rsubst.addPair Rsubst.empty v2 (VAR v1)
  | (APPL (11,[VAR v;x])) ->
    Rsubst.addPair Rsubst.empty v x
  | (APPL (11,[x;VAR v])) ->
    Rsubst.addPair Rsubst.empty v x
  ;;
...
(*fun reduce_case x t ((VAR y,l)::_) =
    [Rsubst.subst (Rsubst.addPair empty y x) l]
...
let rec builtin rewrite env e = match e with
  ...
  | (APPL (12,[(MARKED_VAR x);e])) ->
    if (Rcontext.getMarkedVars e)=[] then
        (if (List.mem x (Rcontext.getMarkedVars e)) then
            [APPL (intern_true,[])]
        else
            [])
    else
...
[APPL (intern_not,[QUANT (intern_exists,v,(APPL (intern_or,[e;(APPL (intern_not,[p]))])),(APPL (intern_true,[])))])]
  ...
  | (QUANT (15,[(vv,t)],APPL (11,[REF v;e]),p)) ->
    ...
    | (VAR v) ->
        if v=vv && not(List.mem v (Rcontext.getFreeVars e)) then
            [APPL (intern_true,[])]
        else
            []
    | _ -> [])
  ...
  | (QUANT (15,[(vv,t)],APPL (11,[e;VAR v]),p)) ->
    if v=vv && not(List.mem v (Rcontext.getFreeVars e)) then
        ...
        []
  | (QUANT (15,v,e,p)) ->
    let vars = (Mylist.intersect ((Rcontext.getFreeVars e)@(Rcontext.getFreeVars p)) (strip v))
    in
        if vars=[] then
            [e]
               ...
               | (APPL (9,l)) ->
                    ...
                    in
                        if eq=NOEXP then
                            ...
                            [QUANT (intern_exists,(unstrip vars),Rsubst.subst (make_subst vars eq) e,p)]
               | _ -> [])
  | (APPL (75,[QUANT (73,[(v,_)],e,_);term])) ->
    [Rsubst.subst (Rsubst.addPair Rsubst.empty v term) e]
  ...
  | (QUANT (q,v,e,p)) ->
    let v1 = remove_quant_vars ((Rcontext.getFreeVars e)@(Rcontext.getFreeVars p)) v
    in
