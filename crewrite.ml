...
let old_relevant_rule env exp rule =
   (try let REF n = ExpIntern.intern_exp (Env.isACorC env) rule
    ...
    let exp_constants = (Mylist.remove_dups (compute_constants exp))@
                        (List.map (fun (x) -> (MARKED_VAR x)) (Context.getFreeVars exp)) in
...
let rec relevant_rule env e r = match (e,r) with
  ...
  | (exp,(APPL (_,[l;r;c]))) ->
    if has_default exp then true else let l = Context.markVars l
    in
        (match l with
...
let rec filter_rule_list env exp rules =
    ...
    let _ = Trace.trace_list "rewriteRule" (fun (x) -> (List.map (fun (x) -> "dr " ^ (prExp x)) dfcl))
    in
...
let junction_filter_rule_list env exp rules =
    let dfcl = List.fold_left List.append [] (List.map (fun (x) -> get_default_clause (REF x)) rules) in
    let _ = Trace.trace_list "rewriteRule" (fun (x) -> (List.map (fun (x) -> "dr " ^ (prExp x)) dfcl)) in
...
let add_rule rewrite env exp rule =
    (Trace.trace "rewrite" (fun (x) -> "Adding " ^ (prExp exp) ^ "\n") ;
...
let create_rules rewrite env e n = match (e,n) with
  | ((APPL (9,l)),n) ->
    add_rule rewrite env (List.nth l n) (APPL (intern_oriented_rule,[(APPL (intern_and,List.map Context.markVars (Mylist.delete_nth l n)));
                                      (APPL (intern_true,[]));
                                      (APPL (intern_true,[]))]))
  | ((APPL (10,l)),n) ->
    add_rule rewrite env (List.nth l n) (APPL (intern_oriented_rule,[(APPL (intern_or,List.map Context.markVars (Mylist.delete_nth l n)));
                              (APPL (intern_false,[]));
                              (APPL (intern_true,[]))]))
  | ((APPL (18,[c;e1;e2])),1) ->
    add_rule rewrite env e1 (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_true,[]));(APPL (intern_true,[]))]))
  | ((APPL (18,[c;e1;e2])),2) ->
    add_rule rewrite env e2 (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_false,[]));(APPL (intern_true,[]))]))
  | ((APPL (1,[l;r;c])),0) ->
    add_rule rewrite env l (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_true,[]));(APPL (intern_true,[]))]))
  | ((APPL (1,[l;r;c])),1) ->
    add_rule rewrite env l (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_true,[]));(APPL (intern_true,[]))]))
  | ((APPL (2,[l;r;c])),0) ->
    add_rule rewrite env l (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_true,[]));(APPL (intern_true,[]))]))
  | ((APPL (2,[l;r;c])),1) ->
    add_rule rewrite env r (APPL (intern_oriented_rule,[Context.markVars c;(APPL (intern_true,[]));(APPL (intern_true,[]))]))
  | (_,_) -> env
  ;;
