...
let rec mark_vars env x =
   (try let (REF n) = ExpIntern.intern_exp (Env.isACorC env) x in
    ...
    let (REF res) = ExpIntern.intern_exp (Env.isACorC env) (Context.markVars (ExpIntern.decode_exp x)) in
...
and rewrite2_front env x =
    ...
    let res = [res1] in
        ...
        (*val _ = flush_out std_out*)
    let _ = Trace.trace "rewrite" (fun (x) -> "Result:") in
    let _ = Trace.indent () in
    let _ = Trace.trace_list "rewrite" (fun (x) -> List.map prExp res) in
    let _ = Trace.trace_list "rewrite" (fun (x) -> List.map (fun (x) -> prExp (ExpIntern.decode_exp x)) res) in
    let _ = (Trace.undent () ; Trace.undent ()) in
    let _ = Trace.trace "rewrite" (fun (xx) -> "end rewrite subterm: " ^ (prExp x)) in
...
and rewrite_front_k (x,env) kb =
    ...
    let _ = Trace.trace "rewrite" (fun (x) -> "Result:") in
    let _ = Trace.indent () in
    let _ = Trace.trace "rewrite" (fun (x) -> prExp res1) in
    let _ = Trace.trace "rewrite" (fun (x) -> prExp (ExpIntern.decode_exp res1)) in
    let _ = (Trace.undent () ; Trace.undent ()) in
    let _ = Trace.trace "rewrite" (fun (xx) -> "end rewrite for: " ^ (prExp x)) in
    let _ = (Trace.indent () ; Trace.undent ()) in
