...
let orient_rule env e = match e with
  | (APPL (f,[l;r;c])) ->
    (let mvc = Mylist.difference (Context.getFreeVars c) (Context.getFreeVars l) in
    let c1 = Context.markVarsList c mvc in
    let mvc = Mylist.difference (Context.getFreeVars c) (Context.getFreeVars r) in
    let c2 = Context.markVarsList c mvc in
    let mvl = Mylist.difference (Context.getFreeVars l) (Context.getFreeVars r) in
    let l1 = Context.markVarsList l mvl in
    let mvc = Mylist.difference (Context.getFreeVars r) (Context.getFreeVars l) in
    let r1 = Context.markVarsList r mvc in
    ...
    List.mem (APPL (intern_true,[]))
        (List.map ExpIntern.decode_exp
            (rewrite (build_env rewrite env f residue)
                     (Context.unmarkVars (Subst.subst sub cond))))
  ;;
...
let extended_subst sub r vars =
    if vars=[] then
        ...
        let sub2 = List.fold_right
                            (fun v -> (fun s ->
                                Subst.addPair s v
                                    (MARKED_VAR (Context.name_away_from_list
                                        (vars @ (Subst.dom s))
                                        v
                                    ))
                            )) residue sub in
        ...
        if matches=[] then
            ...
            let res =
                    List.map (fun (x,l) ->
                            ...
                            let _ = Trace.indent () in
                            let _ = Trace.trace_list "rewriteRule"
...
let pr_theta theta =
    List.fold_left ( ^ ) "" (List.map (fun (x) -> "["^(Intern.decode x)^"->"^(prExp (Subst.apply theta x))^"]") (Subst.dom theta)) ;;

let identity_theta theta =
    let x = Subst.dom theta in
    let rec id l = match l with
          | [] -> true
          | (f::r) -> ((VAR f)=Subst.apply theta f ||
                         (MARKED_VAR f)=Subst.apply theta f) && id r
    in
        id x
    ;;

let rec unify_refine rewrite env c theta =
    let _ = Trace.trace "rewriteRule" (fun (x) -> ("u: " ^ (prExp c))) in
    let _ = Trace.trace "rewriteRule" (fun (x) -> ("t: " ^ (pr_theta theta))) in
    let _ = Trace.trace_list "rewriteRule" (fun (x) -> List.map (fun (x) -> ("cr: " ^ (prExp (ExpIntern.decode_exp (REF x))))) (Env.getContextList env)) in
    ...
    let rec fu l = match l with
          | [] -> Subst.empty
          | (f::r) ->
            let _ = Trace.trace "rewriteRule" (fun (x) -> ("test: " ^ (prExp f))) in
            ...
            let rec fm l = match l with
                  ...
                  | ((APPL (fu,[l;r;cc]))::rest) ->
                    ...
                    let _ = Trace.trace "rewriteRule" (fun (x) -> ("Unified " ^ (prExp l) ^ " " ^ (prExp f))) in
                    let rec fm2 l = match l with
                          | [] -> fm rest
                          | (f::r) ->
                            let (_,theta2,_,_) = f in
                            let itheta = identity_theta theta2 in
                            let _ = Trace.trace "rewriteRule" (fun (x) -> pr_theta theta2) in
                            let c = Subst.subst theta2 c in
                            let _ = Trace.trace "rewriteRule" (fun (x) -> ("c = " ^ (prExp c))) in
                            let rr = if itheta then c else List.hd ((rewrite env c)@[c]) in
                            let comp = Subst.compose theta2 theta in
                            let _ = Trace.trace "rewriteRule " (fun (x) -> ("c = " ^ (prExp c) ^ " rr = " ^ (prExp rr))) in
                            let res = if itheta || (ExpIntern.decode_exp rr)=(APPL (intern_true,[])) then
                                          comp
                                      else
                                          unify_refine rewrite env c comp
                            in
                                if itheta || res=Subst.empty then fm2 r else res
                    in
                        fm2 mat
            in
                fm rules
...
let unify_refine_cache rewrite env c =
    ...
    in
        ...
        (let theta = unify_refine rewrite env c Subst.empty
         in
    ...
    in
        if matches=[] then
            ...
            let _ = Trace.trace "rewriteRule"
                      (fun (xx) -> "Match3 count: " ^ (string_of_int (List.length matches3))) in
            let _ = (Trace.indent () ; Trace.undent ()) in
            let matches4 = List.map (fun (s,r) -> (Subst.addNormals s,r)) matches3 in
            let res =
                    List.map (fun (x,l,u) ->
                            let res = Context.unmarkUnnormalVars (extended_subst x (Subst.subst u r) vars) in
                            let _ = Trace.trace "rewriteRule"
                                        (fun (xx) -> ("r: " ^  (prExp res))) in
                            let _ = Trace.indent () in
                            let _ = Trace.trace_list "rewriteRule"
                        ...
                        (List.fold_left List.append [] (List.map
                         (fun (x,ll) ->
                          let c = Subst.subst x c in
                          let c1 = ExpIntern.intern_exp (Env.isACorC env) c in
                          let _ = Trace.trace "rewriteRule" (fun (x) -> "START UNIFY") in
                          ...
                          let _ = Trace.trace_list "rewriteRule" (fun (x) -> List.map (fun (r) -> "pr: " ^ (prExp (ExpIntern.decode_exp (REF r)))) (Env.getContextList env)) in
                          let _ = Trace.trace_list "rewriteRule" (fun (x) -> List.map (fun (r) -> "r: " ^ (prExp (ExpIntern.decode_exp (REF r)))) l) in
                          let s = unify_refine_cache rewrite env (map_def c) in
                          let _ = Trace.trace "rewriteRule" (fun (x) ->"DONE UNIFY") in
                              if s=Subst.empty then [] else [(x,ll,s)]
                         ) matches4)
                        ) in
            let _ = Trace.trace "rewriteRule"
                      (fun (xx) -> "Result count: " ^ (string_of_int (List.length res))) in
  ...
  | (APPL (s,[l;r;c])) ->
    ...
    in
        if matches=[] then
            ...
            let res =
                    List.map (fun (x,l) ->
                            let res = Context.unmarkUnnormalVars (extended_subst x r vars) in
                            let _ = Trace.trace "rewriteRule"
                                    (fun (xx) -> ("r: " ^  (prExp res))) in
                            let _ = Trace.indent () in
                            let _ = Trace.trace_list "rewriteRule"
...
let introduced_vars x =
    ...
    let iv = Mylist.remove_dups (Mylist.difference (Context.getFreeVars c) (Context.getFreeVars l)) in
...
let rewriteRule rewrite env e vars =
    ...
    let rules0 = (try Cache.get_rules num with Cache.NoEntry ->
        ...
        let _ = Trace.trace_list "rewriteRuleDisc" (fun (x) -> List.map prExp (Env.getAllRules env)) in
        let _ = Trace.trace "rewriteRuleDisc" (fun (x) -> Disc.makedcstring (getRules env)) in
        let _ = Trace.trace "rewriteRule" (fun (x) -> "Rules:") in
        let _ = Trace.indent () in
        let _ = Trace.trace "rewriteRuleDisc" (fun (x) -> ("P: " ^ (Disc.makedcstring (getRules env)))) in
        let _ = Trace.trace_list "rewriteRule" (fun (xx) -> List.map (fun (x,_) -> prExp (ExpIntern.decode_exp (REF x))) rules) in
        let _ = Trace.undent () in
...
let forceRewrites rewrite env e =
    ...
    let _ = Trace.trace "rewriteRule" (fun (x) -> "Force rules") in
    let _ = Trace.trace_list "rewriteRuleDisc" (fun (x) -> List.map prExp (Env.getAllRules env)) in
    let _ = Trace.trace "rewriteRuleDisc" (fun (x) -> Disc.makedcstring (getRules env)) in
    let _ = Trace.trace "rewriteRule" (fun (x) -> "Rules:") in
    let _ = Trace.indent () in
    let _ = Trace.trace "rewriteRuleDisc" (fun (x) -> ("P: " ^ (Disc.makedcstring (Env.getRules env)))) in
    let _ = Trace.trace_list "rewriteRule" (fun (xx) -> List.map prExp rules) in
    let _ = Trace.undent () in
