...
let rec bsubst b u e = match e with
  ...
  | (CASE (e,t,c)) ->
    let
        c2 = List.map (fun (p,e) ->
                         let bbb = List.append b (Context.getFreeVars p)
                         in
                              (p,bsubst bbb u e)
                     ) c
    in
  ...
  | (LET (v,t,e,bb)) ->
    let
        bbb = List.append b (Context.getFreeVars v)
    in
        (LET (v,t,bsubst b u e,bsubst bbb u bb))
  | (CASE (e,t,c)) ->
    let bbb = List.append b (Context.getFreeVars e) in
    let c2 = List.map (fun (p,e) ->
                         (p,
                          let bb = List.append b (Context.getFreeVars p) in bsubst bb u e)
                     ) c
    in
        (CASE (bsubst b u e,t,c2))
